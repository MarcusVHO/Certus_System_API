import { createUser, findUser, findUserByIdInDb, getRefreshToken, updateRefreshToken } from "../../models/users/usersModel.js";
...
export async function userLogin(req, res) {
  try {
    ...
    try {
      ...
      const refreshToken = jwt.sign({id: user.id}, process.env.REFRESH_TOKEN_SECRET, {expiresIn: "9h"})

      //atualizar refreshToken no banco de dados
      updateRefreshToken(refreshToken, user.id)

      //retorna ambos ao usuario
      res.status(200).json({message: "Login efetuado com sucesso", token, refreshToken})
...
export async function getNewToken(req, res) {
  try {
    ...
    if(tokenDb != refresh_token) {
      ...
      //cria token e refresh token
      const token = jwt.sign({id: user.id, role: user.role}, process.env.SECRET, {expiresIn: "30m"})
      const refreshToken = jwt.sign({id: user.id}, process.env.REFRESH_TOKEN_SECRET, {expiresIn: "9h"})
      ...
      res.status(200).json({message: "Refresh Login efetuado com sucesso", token, refreshToken})
